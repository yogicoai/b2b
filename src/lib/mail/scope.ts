 ...
 *    그 경로로 남의 메일이 보인다.
 *
 * ── 예외: 리드에 붙은 메일은 계정을 가리지 않는다 ──────────────────
 *
 * InboundMail.leadId 가 채워졌다는 것은 "이 회사와의 업무 대화" 라는 뜻이고,
 * 그건 개인 메일이 아니라 **회사 기록**이다. 마케팅팀원이 보낸 콜드메일에 온
 * 답장을 이사님이 못 보면 리드 파이프라인을 같이 쓸 수 없다.
 * (대표님 지시 2026-09-16: "리드 파이프라인은 hjs 안에서의 내용이 jay 한테서
 *  동일하게 보이게… 답장 받음 같은 수치만 나오게 해주면 된다")
 *
 * 그래서 **leadId 로 거는 InboundMail 조회에는 mailFilter 를 붙이지 않는다.**
 * 본문·통수·마지막 수신일·기한 전부 해당한다.
 * leadId 가 없는 메일(개인 메일함)은 지금처럼 계정별로 갈린다.
 *
 * 이 규칙이 한 곳에 안 적혀 있어서 세 번 연달아 누락됐다 —
 * /api/mail/thread(9/16 수정) → /api/leads/relationships → scopeMailStats.
 * 새 라우트에서 leadId 로 InboundMail 을 셀 때 mailFilter 를 넣고 싶어지면
 * 여기를 먼저 읽을 것.
 */
