...
async function openConversationModal(leadId) {
  ...
  root.innerHTML = `
    <div style="background:#ffffff;border-radius:14px;max-width:1140px;width:100%;max-height:90vh;
      ...
      <!-- 대화와 회신을 좌우로 나눈다.
           세로로 쌓았을 때는 회신 칸(초안 생성·한글 대역본·에디터)이 커서 위쪽
           대화를 밀어냈고, 화면이 좁으면 회신 칸 자체의 머리가 잘려 나갔다.
           둘 다 각자 스크롤되게 두면 어느 쪽도 잘리지 않는다. -->
      <div class="conv-split" style="flex:1;display:grid;grid-template-columns:1fr 1fr;
                  min-height:0;overflow:hidden">
        <div style="padding:16px 20px;overflow-y:auto;background:#fcfcfd;min-height:0">
          ${alertBar}
          ${tl.length ? bubbles : '<div style="text-align:center;color:#94a3b8;padding:40px;font-size:13px">아직 주고받은 메일이 없습니다.</div>'}
        </div>
        <div style="border-left:1px solid #e2e8f0;overflow-y:auto;min-height:0;background:#fff">
          ${replyBoxHtml(lastInbound) || '<div style="padding:40px 20px;text-align:center;color:#94a3b8;font-size:13px">받은 메일이 있어야 회신할 수 있습니다.</div>'}
        </div>
      </div>
    </div>`;
...
function replyBoxHtml(lastInbound) {
  ...
  return `
    <div style="background:#ffffff;padding:14px 18px 18px">
      ...
      <div class="reply-2col" style="display:grid;grid-template-columns:1fr;gap:12px;align-items:start">
        <div>
...
function renderUserGuidePage() {
  ...
  // 메뉴를 고칠 때는 여기도 같이 고칠 것.

  const card = (n, icon, title, lead, doList, note, tone) => `
    <div style="background:var(--bg-surface);border:1px solid var(--border-default);border-radius:14px;
      ...
      <div style="min-width:40px;height:40px;background:${tone || 'linear-gradient(135deg,#3FA6D3,#2b7fa6)'};
                  color:#fff;border-radius:11px;display:flex;align-items:center;justify-content:center;
                  font-size:15px;font-weight:800;flex-shrink:0">${n}</div>
      <div style="flex:1;min-width:0">
  ...
  const sectionTitle = (t, sub) => `
    <div style="margin:18px 0 2px">
      <div style="font-size:13px;font-weight:800;color:var(--text-primary)">${t}</div>
      ${sub ? `<div style="font-size:12px;color:var(--text-tertiary);margin-top:2px;line-height:1.7">${sub}</div>` : ''}
    </div>`;

  els.content.innerHTML = `
    <div style="max-width:900px;margin:0 auto;display:flex;flex-direction:column;gap:13px;padding-bottom:40px">

      <!-- ── 한 줄 요약 ── -->
      <div style="padding:24px 26px;border-radius:16px;
                  background:linear-gradient(135deg,#eaf6fb 0%,#d3ebf6 100%);border:1px solid #9ed3e9">
        <h2 style="margin:0 0 8px;font-size:21px;font-weight:800;color:#0d3b4f">
          업체를 찾아 메일을 보내고, 답장까지 한곳에서 관리합니다
        </h2>
        <div style="font-size:13.5px;color:#14566f;line-height:1.85">
          왼쪽 메뉴는 <b>세 묶음</b>입니다 — ① 메일함 ② 리드 파이프라인 ③ 설정·도구.<br>
          <b>고른 곳에만 메일이 나갑니다.</b> 목록에 있다고 저절로 나가지 않습니다.
        </div>
        <div style="margin-top:13px;padding:12px 15px;background:#fff;border-radius:10px;
                    font-size:12.5px;color:#0d3b4f;line-height:1.9">
          🔎 <b>크롤링</b> → ✅ <b>AI 검증 완료</b> → 📨 <b>발송 관리</b> →
          💬 답장 받음 → 🤝 대화 진행 중 → ⭐ 파트너십 확정
        </div>
        <div style="margin-top:8px;font-size:12px;color:#14566f">
      ...
      ${card('①', '📥', '받은 메일함 — 밤새 뭐가 왔나',
        ...
        '· <b>📋 오늘의 브리핑</b> — 위 세 가지를 한 장으로. 매일 아침 같은 내용이 메일로도 갑니다',
        '리드에 없는 곳에서 온 메일도 여기엔 다 뜹니다. 파이프라인은 <b>회사</b> 기준이고, 메일함은 <b>메일</b> 기준이라 서로 보는 각도가 다릅니다.'
      )}

      ${card('②', '🔎', '크롤링 — 업체 찾기',
        '카테고리를 고르고 <b>[확인하고 시작]</b> 을 누르면 끝입니다. 전국 17개 시·도를 돌면서 업체를 찾고, 홈페이지에 들어가 메일 주소를 뽑고, 규모가 맞는지 AI 가 가립니다. <b>중간에 확인하실 것은 없습니다.</b>',
        '· <b>키워드는 비워 두세요</b> — 카테고리에 등록된 것이 그대로 쓰입니다<br>' +
        '· 누르기 전에 <b>몇 곳을 찾고 · 얼마가 들고 · 몇 분 걸리는지</b> 미리 보여줍니다<br>' +
        '· 도는 동안 업체 이름이 한 줄씩 실시간으로 쌓입니다<br>' +
        '· 끝나면 그 카테고리가 선택된 채로 <b>AI 검증 완료</b> 화면으로 넘어갑니다',
        '한 번 돌면 <b>수십 분</b>이 걸립니다. 다른 화면을 보다 오셔도 계속 돌고 있고, 이 화면으로 돌아오면 진행 상황이 그대로 보입니다. 메일 주소를 못 찾은 곳은 <b>검증 실패</b>에 사유와 함께 남습니다 — 나중에 전화로 딸 수 있게요.'
      )}

      ${card('③', '✅', 'AI 검증 완료 — 보낼 곳 고르기',
        'AI 가 <b>규모</b>를 보고 통과시킨 곳입니다. 업종이 맞아도 1인 펜션·동네 의원처럼 <b>한두 개 사고 끝날 곳</b>은 여기 오지 않습니다.',
        '· 목록 위 <b>분류 탭</b>으로 학교·기업·병의원·리조트·스포츠를 나눠 봅니다<br>' +
        '· 회사를 눌러 판정 근거와 점수를 확인<br>' +
        '· 보낼 곳을 골라 <b>[발송 리스트로 옮기기]</b><br>' +
        '· 아니다 싶으면 <b>검증 실패</b>로 내려보냅니다',
        'AI 판정은 틀릴 수 있습니다. 실제로 규모 있는 곳이 잘못 걸러진 적이 있어서, <b>검증 실패</b>에서도 되돌릴 수 있게 해뒀습니다.'
      )}

      ${card('④', '📨', '발송 관리 — 실제로 보내기',
        '칸이 셋입니다. <b>보낼 메일</b>(고른 곳) · <b>예약 발송</b>(시각이 잡힌 것) · <b>발송 완료</b>(나간 것).',
        '· 분류 탭에서 <b>[리조트·호텔 139곳]</b> 처럼 골라 보냅니다<br>' +
        '· 버튼에 <b>무엇을 보내는지</b> 그대로 나옵니다 — “✉️ 리조트·호텔 139곳에 보내기”<br>' +
        '· 분류마다 <b>다른 문구</b>가 나갑니다. 전체로 두고 보내면 업체마다 그 분류의 양식이 각각 적용됩니다<br>' +
        '· 문구는 <b>📝 메일 양식</b> 에서 분류별로 고칩니다',
        '<b>한 번에 다 나가지 않습니다.</b> 기본은 30곳씩 30분 간격입니다. 짧은 시간에 수백 통을 보내면 보내는 계정이 잠기고, 그러면 그날 발송이 통째로 멈춥니다 — 실제로 겪은 일입니다.'
      )}

      ${card('⑤', '💬', '답장 받음 → 대화 진행 중 → 파트너십 확정',
        '상대가 답하면 <b>자동으로</b> 답장 받음으로 옮겨집니다. 손댈 것 없습니다.',
        '· <b>답장 받음</b> — 회신이 온 곳. 회사명 옆 💬 로 주고받은 메일을 봅니다<br>' +
        '· <b>대화 진행 중</b> — 조건·일정·가격을 주고받는 단계. 직접 옮깁니다<br>' +
        '· <b>파트너십 확정</b> — 계약이 끝난 곳. 여기 오면 광고 메일이 더 안 나갑니다',
        '대화가 시작된 곳에는 자동 발송이 멈춥니다. 협상 중인 거래처에 콜드메일이 또 가면 곤란하니까요.'
      )}

      ${sectionTitle('알아두실 것')}

      ${card('!', '⚖️', '광고 메일에는 법이 걸려 있습니다',
        '우리가 먼저 보내는 메일은 전부 <b>광고성 정보</b>입니다. 정보통신망법 제50조가 세 가지를 요구합니다.',
        '· 제목 맨 앞 <b>(광고)</b> 표기<br>' +
        '· 본문에 <b>발신자 정보</b> — 상호·주소·연락처<br>' +
        '· <b>수신거부 링크</b> — 본문에 넣고, 실제로 동작할 것',
        '셋 다 <b>발송할 때 자동으로 붙습니다.</b> 신경 쓰지 않으셔도 됩니다. 야간(21시~08시)에는 아예 나가지 않도록 막혀 있고, 한 번 수신거부한 곳은 다시 수집돼도 발송 대상에서 빠집니다.',
        'linear-gradient(135deg,#f59e0b,#d97706)'
      )}

      ${card('!', '🖥', '크롤링은 이 컴퓨터에서만 돕니다',
        '크롤링은 실제 브라우저를 띄워 홈페이지를 열어봅니다. 배포된 서버에는 그 브라우저가 없어서 <b>서버에서는 크롤링이 안 됩니다.</b>',
        '· 크롤링 → <b>사무실 컴퓨터</b>에서 실행<br>' +
        '· 검토·발송·메일함 → 어디서든 (배포된 주소로)',
        '같은 데이터베이스를 보기 때문에, 여기서 크롤링을 돌리면 결과가 <b>배포된 화면에도 바로</b> 뜹니다.',
        'linear-gradient(135deg,#6b7280,#4b5563)'
      )}

      ${card('!', '🧭', '헷갈리기 쉬운 두 곳',
        '<b>[⚠️ 회신 필요]</b> 와 <b>[💬 답장 받음]</b> 은 같은 일을 가리킬 때가 있습니다. 보는 각도가 달라서입니다.',
        '· <b>회신 필요</b> — “답장 안 한 메일이 남았나” (빠뜨림 방지 · 메일 단위)<br>' +
        '· <b>답장 받음</b> — “이 회사와 어디까지 갔나” (영업 진행 · 회사 단위)',
        null,
        'linear-gradient(135deg,#8b5cf6,#7c3aed)'
      )}

      ${sectionTitle('자주 묻는 것')}

      <div style="background:var(--bg-surface);border:1px solid var(--border-default);
                  border-radius:14px;padding:6px 22px 14px">
        ${[
          ['메일이 저절로 나가나요?',
           '아니요. 고른 곳에만 나갑니다. 예약을 걸어 둔 건은 정해진 시각에 나가고, <b>[📅 예약 발송]</b> 칸에서 언제든 취소할 수 있습니다.'],
          ['크롤링이 멈춘 것 같아요.',
           '홈페이지 한 곳을 여는 데 몇 초씩 걸려서 숫자가 천천히 올라갑니다. 아래 실시간 목록에 업체 이름이 계속 쌓이면 정상입니다.'],
          ['메일 주소를 못 찾은 곳은 버려지나요?',
           '아니요. <b>검증 실패</b>에 “메일 주소를 찾지 못했습니다” 사유와 함께 남습니다. 전화번호는 있으니 나중에 따로 연락할 수 있고, 다음에 크롤링해도 같은 곳을 또 뒤지지 않습니다.'],
          ['같은 업체에 몇 번까지 보내나요?',
           '최대 3회, 그것도 48시간 간격이 지나야 나갑니다. 답장이 오면 그 즉시 멈춥니다.'],
          ['분류마다 다른 문구가 정말 나가나요?',
           '네. 병원에는 대기실 이야기가, 호텔에는 로비 이야기가 나갑니다. 문구는 <b>[📝 메일 양식]</b> 에서 분류별로 고치시면 됩니다.'],
          ['보낸 메일이 보낸메일함에 남나요?',
           '네. 발송할 때 이카운트 보낸메일함에도 사본을 남깁니다. 웹메일·아웃룩에서도 보입니다.'],
        ].map(([q, a]) => `
          <details style="border-bottom:1px solid var(--border-subtle);padding:12px 0">
            <summary style="font-size:13.5px;font-weight:700;color:var(--text-primary);cursor:pointer;list-style:none">
              <span style="color:#3FA6D3;margin-right:6px">Q</span>${q}
            </summary>
            <div style="font-size:12.5px;color:var(--text-secondary);line-height:1.85;margin-top:8px;padding-left:18px">${a}</div>
          </details>`).join('')}
      </div>

      <div style="text-align:center;font-size:12px;color:var(--text-quaternary);margin-top:6px;line-height:1.8">
        막히는 곳이 있으면 화면 이름을 그대로 알려 주세요 — 어디서 막혔는지 바로 찾을 수 있습니다.
      </div>
    </div>`;
}
